...
# Map original WhatsApp human recordings directly to intent MP3 filenames
MAP = {
    ...
    "WhatsApp Audio 2026-08-01 at 10.42.28 AM.aac": ["number_source.mp3"]
}
...
def convert_to_mp3(src_path: Path, dest_mp3_path: Path):
    ...
    if res.returncode == 0 and dest_mp3_path.exists():
        print(f" -> Installed Real Human Voice: {src_path.name} => {dest_mp3_path.name} ({dest_mp3_path.stat().st_size} bytes)")
        return True
    else:
        print(f" -> Failed to convert {src_path.name}: {res.stderr.decode('utf-8', errors='ignore')}")
        return False

def process():
    print("==================================================")
    print(" RESTORING REAL HUMAN WHATSAPP RECORDINGS 100%    ")
    print("==================================================")
    ...
    for src_name, target_mp3s in MAP.items():
        ...
        if not src_path.exists():
            print(f"Backup file missing: {src_name}")
            continue
    ...
    print("\n==================================================")
    print(" 100% REAL HUMAN WHATSAPP RECORDINGS RESTORED!   ")
    print("==================================================")
